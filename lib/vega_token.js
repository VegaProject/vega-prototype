...
export default class {
  ...
  countVotes(offerId) {
    ...
    return this.project.countVotes(offerId).then(function(result) {      
      console.log('Vote status (Yes/No/Quorum)' + result );
    }).catch(function(e) {
      console.log(e);
      console.log("Error establishing Offer; see log.");
    });
  ...
  getOfferStatus(offerId){
    ...
    return this.project.getOfferStatus(offerId).then(function(result) {      
      console.log('Offer status (Passed?/creatorsDeposit/finder) ' + result  );
    }).catch(function(e) {
      console.log(e);
      console.log("Error establishing Offer; see log.");
    });     
  }
  calcCDR(_currentValue, _startingValue, _vegaPeriods, _stake, _absTotalRewards, _tokenConversion, _currentBalance){
        console.log("Initiating CDR calculations... (All Number crunching powered by witchcraft...)");
    return this.cdr.calculateCDRForPositiveOutcome(_currentValue, _startingValue, _vegaPeriods, _stake, _absTotalRewards, _tokenConversion, _currentBalance).then(function(result) {      
