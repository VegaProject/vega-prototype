...
contract Finders is Ownable {
    ...
    returns (uint offerId)
  {
    ...
    o.executed = false;
    o.numberOfVotes = 1;
    o.offerHash = sha3(_finders, _description, _openFor, _offerHash);
    return offerId;
  }

  /// @param _id Id of Offer.
  /// @param _finders Finders amount.
  /// @param _openFor Time allowed to vote, must be under 30 days.
  /// @param _offerHash Hash of Offer.
  ...
  {
    Offer o = offers[_id];
    return o.offerHash == sha3(_finders, _description, _openFor, _offerHash);
  }
  ...
  function checkIfOfferCanExecute(uint _id, bytes _transactionBytecode) private constant returns (bool) {
    Offer o = offers[_id];
    if (now < o.openFor || o.executed || o.offerHash != sha3(o.finders, o.description, o.openFor, _transactionBytecode)) throw;
